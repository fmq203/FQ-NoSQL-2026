...
class CorrelationIDMiddleware(BaseHTTPMiddleware):
    """
    Middleware para manejo de correlation IDs para distributed tracing.

    Ingress: Extraer X-Correlation-ID de headers; si no existe, generar UUID v4
    Interno: Usar como correlation_id en logs estructurados
    Egress: Pasar X-Correlation-ID a llamadas HTTP downstream
    Logging: trace_id = correlation_id; span_id = nuevo UUID por operación
    """
